...
//        }
    ...
    public List<Boleto> postBoleto(Boleto boletoBody) throws Exception {
        ...
        if (response.code() == 201) {
            /*String jsonResponse = response.body().string();
            JSONObject objectJsonResponse = new JSONObject(jsonResponse);
            JSONObject arrObjectJsonResponse = objectJsonResponse.getJSONObject("data");

            List<Boleto> boletos = new ArrayList<Boleto>();
            Boleto boleto = deserializePostBoleto(arrObjectJsonResponse);
            boletos.add(boleto);
*/
//
//            for (int i = 0; i < arrObjectJsonResponse.length(); i++) {
//                JSONObject objJSON = (JSONObject) arrObjectJsonResponse.get(i);
//                Boleto boleto = deserializePostBoleto(objJSON);
//                boletos.add(boleto);
//            }
//            return boletos;
             return null;
